...
func produceMessagesAsync(topic string) {
    ...
    go func() {
        for success := range producer.Successes() {
            log.Printf("sent message to partition %d with offset %d with key %s", success.Partition, success.Offset, success.Key)
        }
    }()
...
func (h * ConsumerGroupHandler) ConsumeClaim(session sarama.ConsumerGroupSession,  claim sarama.ConsumerGroupClaim) error {
    for message := range claim.Messages() {
        log.Printf("got message: topic=%s partition=%d offset=%d key=%s: %s", message.Topic, message.Partition, message.Offset, message.Key, string(message.Value))
